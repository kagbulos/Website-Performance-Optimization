...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var numberOfPizzas = document.querySelectorAll(".randomPizzaContainer").length; //save the number of pizzas in a variable so we dont have to recalculate it every time
    var dx = determineDx(document.querySelectorAll(".randomPizzaContainer")[1], size); //we can pull out dx because we dont need to calculate it every time
    var newwidth = (document.querySelectorAll(".randomPizzaContainer")[1].offsetWidth + dx) + 'px'; // even though dx changes, the new width is consistent for all 100 values and can be pulled out
    var allRandomPizzaContainer = document.querySelectorAll(".randomPizzaContainer");//instead of calculating this every time in the for loop, save the list in a variable
...
//because the same 5 values were being created due to mod operator, we store them so they dont have to be recalculated every time
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");
  var zero = 100* Math.sin((document.body.scrollTop / 1250) + (0 ));
  var one = 100* Math.sin((document.body.scrollTop / 1250) + (1 ));
  var two = 100* Math.sin((document.body.scrollTop / 1250) + (2));
  var three = 100* Math.sin((document.body.scrollTop / 1250) + (3));
  var four = 100* Math.sin((document.body.scrollTop / 1250) + (4));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 200; i++) {
    ...
    var tempTop = (Math.floor(i / cols) * s); //this is our temp variable that will store the top of the pizza && will be used to see if pizza is painted on the page
    elem.style.top = tempTop + 'px';

    if (elem.basicLeft < screen.width + 73 && elem.basicLeft > -73 && tempTop < screen.height) //eliminate pizzas that are off the screen to the left, right, and below
    {
